...
const validateCliOptions = (options) => {
  ...
  if (options.apiPath && !options.apiPath.startsWith('/')) {
...
const saveConfig = (configData) => {
  ...
  const config = {
    ...
    email: configData.authType === 'basic' ? configData.email.trim() : undefined
  };
...
const promptForMissingValues = async (providedValues) => {
  ...
  if (!providedValues.authType) {
    questions.push({
      ...
      default: hasEmail ? 'basic' : 'basic'
    });
  }

  // Email question (conditional)
  const determinedAuthType = providedValues.authType || 'basic';
  if (determinedAuthType === 'basic' && !providedValues.email) {
    questions.push({
      type: 'input',
      name: 'email',
...
async function initConfig(cliOptions = {}) {
  ...
  // Determine if we can operate in non-interactive mode
  // Non-interactive requires: domain, token, and either authType or email (for inference)
  const canBeNonInteractive = hasRequiredValues && (
    providedValues.authType || 
    providedValues.email || 
    providedValues.token && providedValues.domain  // default to bearer if only domain+token
  );

  if (canBeNonInteractive && (providedValues.authType || providedValues.email)) {
  ...
  try {
    ...
    // Normalize auth type
    mergedValues.authType = normalizeAuthType(mergedValues.authType || 'basic', Boolean(mergedValues.email));
