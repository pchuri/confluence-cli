...
const normalizeAuthType = (rawValue, hasEmail) => {
  const normalized = (rawValue || '').trim().toLowerCase();
  if (normalized === 'basic' || normalized === 'bearer' || normalized === 'mtls' || normalized === 'cookie') {
    return normalized;
  }
...
const validateCliOptions = (options) => {
  ...
  if (options.authType && !['basic', 'bearer', 'mtls', 'cookie'].includes(options.authType.toLowerCase())) {
...
async function initConfig(cliOptions = {}) {
  ...
  if (!hasCliOptions) {
    ...
    if (answers.authType === 'cookie') {
      configData.cookie = answers.cookie;
    }
  ...
  const hasRequiredValues = Boolean(
    ...
    (
      ...
      || providedValues.authType === 'cookie'
      || (
...
function getConfig(profileName) {
  ...
  if (envDomain && (envToken || envAuthType === 'mtls' || envMtls || envAuthType === 'cookie' || envCookie)) {
    const inferredAuthType = envAuthType || (envMtls && !envToken ? 'mtls' : (envCookie && !envToken ? 'cookie' : undefined));
    ...
    if (authType !== 'mtls' && authType !== 'cookie' && !envToken) {
    ...
    if (authType === 'cookie' && !envCookie) {
      console.error(chalk.red('❌ Cookie authentication requires CONFLUENCE_COOKIE.'));
      process.exit(1);
    }
  ...
  try {
    ...
    if (!trimmedDomain || (authType !== 'mtls' && authType !== 'cookie' && !trimmedToken)) {
    ...
    if (authType === 'cookie' && !trimmedCookie) {
      console.error(chalk.red('❌ Cookie authentication requires a cookie value.'));
      console.log(chalk.yellow('Please rerun "confluence init" to add your cookie.'));
      process.exit(1);
    }
