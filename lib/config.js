...
const { VALID_PLANTUML_FORMATS } = require('./plantuml-format');
...
const normalizePlantumlFormat = (rawValue, source) => {
  if (rawValue === undefined || rawValue === null || rawValue === '') {
    return undefined;
  }
  const value = String(rawValue).trim().toLowerCase();
  ...
  if (!isJsonMode()) {
    console.error(chalk.yellow(
      `⚠ Invalid plantumlFormat ${label}"${rawValue}"; valid values: ${VALID_PLANTUML_FORMATS.join(', ')}. Falling back to "${VALID_PLANTUML_FORMATS[0]}".`
    ));
  }
  return undefined;
};
...
// Reduce a domain to its bare lowercase host: strip the scheme and everything
// from the first slash onward.
...
const validateCliOptions = (options) => {
  ...
  if (options.storeTokenInKeychain && !isKeychainSupported()) {
    errors.push(process.platform === 'darwin'
      ? '--keychain is disabled by CONFLUENCE_KEYCHAIN'
      : '--keychain is only available on macOS');
  }

  if (options.storeTokenInKeychain && normAuthType && !['basic', 'bearer'].includes(normAuthType)) {
    errors.push('--keychain only applies to basic or bearer authentication');
  }
...
const saveConfig = (configData, profileName) => {
  ...
  const trimmedToken = configData.token ? configData.token.trim() : '';
  const keychainLogin = configData.authType === 'basic' && configData.email
    ? configData.email.trim()
    : undefined;
  let storedInKeychain = false;

  if (trimmedToken && configData.storeTokenInKeychain) {
    // Keep the token out of config.json: write it to the macOS Keychain and
    // leave the profile without a `token` so getConfig() falls back to it.
    const { replaced } = setKeychainToken({ host: extractHost(config.domain), login: keychainLogin, token: trimmedToken });
    storedInKeychain = true;
    if (replaced) {
      console.log(chalk.yellow('⚠ Replaced an existing Keychain item for this host; any other profile using it now gets the new token.'));
    }
  ...
  if (storedInKeychain) {
    console.log(`🔐 Token stored in the macOS Keychain (${describeKeychainItem({ host: extractHost(config.domain), login: keychainLogin })}), not in config.json.`);
  }
  console.log(chalk.yellow('\n💡 Tip: You can regenerate this config anytime by running "confluence init"'));
};

// Helper function to prompt for missing values
const promptForMissingValues = async (providedValues, { storeTokenInKeychain = false } = {}) => {
  const questions = [];
  ...
  if (!providedValues.token) {
    ...
    // Offer the macOS Keychain when a token was just typed in and --keychain
    // was not already given.
    questions.push({
      type: 'confirm',
      name: 'storeTokenInKeychain',
      message: 'Store the token in the macOS Keychain instead of config.json?',
      default: false,
      when: (responses) => !storeTokenInKeychain
        && isKeychainSupported()
        && Boolean(responses.token && responses.token.trim())
    });
  }
...
// Resolve a token from the macOS Keychain as a fallback for basic/bearer auth.
// Same matching rules as .netrc: by host and, for basic auth, by email (login).
const resolveKeychainToken = (authType, domain, email) => {
  if (authType !== 'basic' && authType !== 'bearer') {
    return { token: undefined, attempted: false };
  }
  const host = extractHost(domain);
  ...
  const login = authType === 'basic' ? email : undefined;
  return getKeychainToken({ host, login });
};

async function initConfig(cliOptions = {}) {
  const profileName = cliOptions.profile;
  const storeTokenInKeychain = Boolean(cliOptions.keychain);
  ...
  const hasCliOptions = Object.values(providedValues).some(v => v) || storeTokenInKeychain;
  ...
  if (!hasCliOptions) {
    ...
    const configData = { ...answers, readOnly, storeTokenInKeychain: storeTokenInKeychain || Boolean(answers.storeTokenInKeychain) };
  ...
  const validationErrors = validateCliOptions({ ...providedValues, storeTokenInKeychain });
  ...
  try {
    ...
    const mergedValues = await promptForMissingValues(providedValues, { storeTokenInKeychain });
    ...
    const useKeychain = storeTokenInKeychain || Boolean(mergedValues.storeTokenInKeychain);
    if (storeTokenInKeychain && !(mergedValues.token && mergedValues.token.trim())) {
      console.error(chalk.yellow('⚠ --keychain ignored: no token was provided for this profile.'));
    }
    saveConfig({ ...mergedValues, readOnly, storeTokenInKeychain: useKeychain }, profileName);
  } catch (error) {
    console.error(chalk.red(`❌ ${error.message}`));
    process.exit(1);
  }
...
function getConfig(profileName, { throwOnError = false } = {}) {
  ...
  const envPlantumlFormat = normalizePlantumlFormat(process.env.CONFLUENCE_PLANTUML_FORMAT, 'from CONFLUENCE_PLANTUML_FORMAT');
  ...
  if (envDomain && hasEnvAuth) {
    ...
    return {
      ...
      plantumlFormat: envPlantumlFormat
    };
  }
  ...
  try {
    ...
    // Fall back to the macOS Keychain, then ~/.netrc, for the token when the
    // profile has none stored.
    let keychainAttempted = false;
    let netrcAttempted = false;
    if (!trimmedToken) {
      const keychain = resolveKeychainToken(authType, trimmedDomain, trimmedEmail);
      trimmedToken = keychain.token;
      keychainAttempted = keychain.attempted;
    }
    if (!trimmedToken) {
    ...
    if (authErrors.length > 0) {
      ...
      if (netrcAttempted && !trimmedToken) {
        ...
        if (keychainAttempted) {
          sources.push(`macOS Keychain item (${describeKeychainItem({ host: extractHost(trimmedDomain), login: authType === 'basic' ? trimmedEmail : undefined })})`);
        }
        sources.push(`${getNetrcPath()} entry for machine "${extractHost(trimmedDomain)}"`);
        console.log(chalk.yellow(`No profile token found, and no matching ${sources.join(' or ')}.`));
      }
    ...
    const plantumlFormat = envPlantumlFormat
      ?? normalizePlantumlFormat(storedConfig.plantumlFormat, `in profile "${targetProfile}"`);
