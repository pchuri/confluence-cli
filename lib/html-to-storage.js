...
const { encodePlantuml } = require('./plantuml-codec');
...
function convertCodeBlock(node, ctx) {
  ...
  const cdataBody = body.replace(/]]>/g, ']]]]><![CDATA[>');
  switch (language) {
  case 'plantuml':
    if (ctx && ctx.plantumlFormat === 'plantumlcloud') {
      return buildPlantumlCloudMacro(body, ctx);
    }
    return `<ac:structured-macro ac:name="plantuml"><ac:plain-text-body><![CDATA[${cdataBody}]]></ac:plain-text-body></ac:structured-macro>`;
  default:
    return `<ac:structured-macro ac:name="code"><ac:parameter ac:name="language">${language}</ac:parameter><ac:plain-text-body><![CDATA[${cdataBody}]]></ac:plain-text-body></ac:structured-macro>`;
  }
}

// Escape a macro parameter value. Parameter bodies are plain XML text (no
// CDATA), so the four markup-significant characters must be entity-encoded.
// Emitted values are base64 / fixed literals, but this keeps the builder safe
// for any future non-literal parameter.
function escapeMacroParamValue(v) {
  return String(v)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
// Build the `plantumlcloud` macro documented by the PlantUML Diagrams for
// Confluence app: the diagram source travels in a `data` parameter that is
// URI-encoded, raw-deflated and base64-encoded. `filename` names the SVG
// attachment the app renders from and is auto-generated per conversion.
function buildPlantumlCloudMacro(source, ctx) {
  const counter = ctx && ctx.plantumlCounter;
  if (counter) counter.count += 1;
  const index = counter ? counter.count : 1;
  const params = [
    ['toolbar', 'bottom'],
    ['filename', `plantuml-diagram-${index}.svg`],
    ['data', encodePlantuml(source)],
    ['compressed', 'true'],
    ['revision', '1'],
  ];
  const body = params
    .map(([name, value]) => `<ac:parameter ac:name="${name}">${escapeMacroParamValue(value)}</ac:parameter>`)
    .join('');
  return `<ac:structured-macro ac:name="plantumlcloud">${body}</ac:structured-macro>`;
}
...
function htmlToStorage(html, options = {}) {
  ...
  const ctx = {
    linkStyle,
    plantumlFormat: resolvePlantumlFormat({ plantumlFormat: options.plantumlFormat }),
    // Reset per conversion so `plantuml-diagram-<n>.svg` numbering is
    // deterministic for a given input document.
    plantumlCounter: { count: 0 },
    depth: 0,
