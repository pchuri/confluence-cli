...
function readWriteConfig(env = {}) {
  ...
  return Object.freeze({
    enabled: Boolean(writesEnabled && spaceConfig.valid),
    spaces: Object.freeze(new Set(spaceConfig.spaces)),
    limits: limitConfig.limits,
    limitsValid: limitConfig.valid,
  });
}
...
function assertWriteEnabled(env = {}) {
  ...
  if (!config.limitsValid) {
    throw makeError('INVALID_LIMITS', 'Confluence Pi payload limits are invalid.');
  }
...
function normalizeAllowedSpaces(allowedSpaces) {
  if (allowedSpaces instanceof Set || Array.isArray(allowedSpaces)) {
    return new Set(Array.from(allowedSpaces, (space) => String(space).trim().toUpperCase()).filter(Boolean));
  }
  return new Set();
}
...
function resolveProjectReadOutputPath(projectRoot, candidate) {
  const root = canonicalProjectRoot(projectRoot);
  const outputPath = resolveProjectOutputPath(root, candidate);
  const gitDirectory = path.join(root, '.git');

  if (isInsideProject(gitDirectory, outputPath)) {
    throw projectError('Output path must not be inside the project Git metadata directory.');
  }

  return outputPath;
}

function resolveProjectNewOutputFile(projectRoot, candidate) {
  const outputPath = resolveProjectReadOutputPath(projectRoot, candidate);
  if (fs.existsSync(outputPath) && fs.statSync(outputPath).isFile()) {
    throw projectError('Output path must not overwrite an existing regular file inside the project directory.');
  }
  return outputPath;
}
...
function normalizeBodySource(input, projectRoot, limits, snapshots, required) {
  ...
  if (hasContent) {
    ...
    const bodyBytes = byteLength(serialized);
    ensureWithinLimit(bodyBytes, limits.maxBodyBytes, 'Body');
    return {
      present: true,
      input: { content: serialized, bodyBytes },
    };
  }
  ...
  return {
    present: true,
    input: { contentFile: canonical, bodyBytes },
  };
}
...
function normalizePropertyValue(input, projectRoot, limits, snapshots) {
  ...
  if (hasValue) {
    ...
    const propertyBytes = byteLength(serialized);
    ensureWithinLimit(propertyBytes, limits.maxPropertyBytes, 'Property value');
    return {
      present: true,
      input: { value: serialized, propertyBytes },
    };
  }
  ...
  try {
    ...
    throw makeError('PAYLOAD_INVALID', 'Property value file must contain valid JSON.');
  }
  const propertyBytes = byteLength(JSON.stringify(parsed));
  ensureWithinLimit(propertyBytes, limits.maxPropertyBytes, 'Property value');
  snapshots.push(snapshotFile(canonical));
  return {
    present: true,
    input: { valueFile: canonical, propertyBytes },
  };
}
