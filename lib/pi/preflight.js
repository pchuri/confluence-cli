...
async function handleMove(invoker, input) {
  ...
  if (source.spaceKey !== destination.spaceKey) {
...
function normalizeSpaceRecord(value) {
  ...
  const spaceKey = requireSpaceKey(value.key ?? value.spaceKey, 'space key');
  const title = requireText(value.name ?? value.title, 'space name');
  return { spaceKey, title };
}

async function handleCreate(invoker, input) {
  const requestedSpace = requireSpaceKey(input.spaceKey, 'space key');
  const response = await invokeJson(invoker, 'confluence_spaces', { all: true });
  const candidates = response && typeof response === 'object'
    ? (Array.isArray(response.spaces) ? response.spaces : response.results)
    : undefined;
  if (!Array.isArray(candidates)) {
    throw makeError('MALFORMED_RESULT', 'Preflight spaces response must include a spaces array.');
  }
  const matches = candidates
    .map((entry) => normalizeSpaceRecord(entry))
    .filter((space) => space.spaceKey === requestedSpace);
  if (matches.length === 0) {
    throw makeError('SPACE_NOT_FOUND', `Destination space ${requestedSpace} was not found or is inaccessible.`);
  }
  if (matches.length !== 1) {
    throw makeError('SPACE_AMBIGUOUS', `Destination space ${requestedSpace} is ambiguous.`);
  }
...
function normalizePlannedTree(value, childCount) {
  if (!Array.isArray(value) || value.length !== childCount) {
    throw makeError('MALFORMED_RESULT', 'Preflight copy tree response must include the complete planned tree.');
  }
  const seenIds = new Set();
  const records = value.map((entry) => {
    if (!entry || typeof entry !== 'object') {
      throw makeError('MALFORMED_RESULT', 'Preflight planned tree entries must be objects.');
    }
    const id = requireText(entry.id ?? entry.pageId, 'planned page id');
    if (seenIds.has(id)) {
      throw makeError('MALFORMED_RESULT', 'Preflight planned tree page IDs must be unique.');
    }
    seenIds.add(id);
    const parentId = requireText(entry.parentId, 'planned parent id');
    const title = requireText(entry.title, 'planned page title');
    const version = Number(entry.version && typeof entry.version === 'object' ? entry.version.number : entry.versionNumber ?? entry.version);
    if (!Number.isSafeInteger(version) || version < 1) {
      throw makeError('MALFORMED_RESULT', 'Preflight planned page versions must be positive integers.');
    }
    return { id, parentId, title, version };
  });
...
async function handleCopyTreePreview(invoker, input, operation) {
  ...
  const plannedTree = normalizePlannedTree(preview.plannedTree, childCount);
  const plannedTreeFingerprint = stableFingerprint(plannedTree);
