...
function normalizeSpaceKey(value, name) {
  return requireText(value, name);
}

function sameSpaceKey(left, right) {
...
function normalizePageRecord(payload, label = 'page') {
  ...
  const versionValue = data.version && typeof data.version === 'object'
    ? data.version.number
  ...
  const record = {
    ...
    spaceKey: normalizeSpaceKey(space, 'space key'),
  };
  if (hasValue(versionValue)) {
    const versionNumber = Number(versionValue);
    if (!Number.isSafeInteger(versionNumber) || versionNumber < 1) {
      throw makeError('MALFORMED_RESULT', `Preflight ${label} version must be a positive integer.`);
    }
...
function normalizeCreateInput(rawInput) {
  ...
  if (hasValue(input.spaceKey)) {
    input.spaceKey = normalizeSpaceKey(input.spaceKey, 'space key');
  }
  return input;
}
...
function bindCanonicalPageId(input, field, page, label) {
  const requested = normalizeId(input[field]);
  if (!isUrlTarget(requested) && requested !== page.pageId) {
    throw makeError('TARGET_MISMATCH', `Preflight ${label} response ID does not match the requested target.`);
  }
  input[field] = page.pageId;
}
...
function verifyDirectOwnership(response, targetId, page, label) {
  ...
  const returnedId = requireText(response.id, `${label} ID`);
  const returnedPageId = requireText(response.pageId, `${label} page ID`);
  if (returnedId !== String(targetId)) {
    throw makeError('TARGET_MISMATCH', `Preflight ${label} response ID does not match the requested target.`);
  }
  if (returnedPageId !== page.pageId) {
    throw makeError('TARGET_MISMATCH', `Preflight ${label} response does not belong to the requested page.`);
  }
}

async function findPagedMatch({ invoker, toolName, page, label, type, matchValue, required = true }) {
...
function payloadDetails(input, { includeType = false } = {}) {
  const details = [];
  if (Number.isSafeInteger(input.bodyBytes) && input.bodyBytes >= 0) details.push(`${input.bodyBytes} bytes`);
  if (hasValue(input.format)) details.push(`format: ${input.format}`);
  if (includeType) details.push(`type: ${input.type ?? 'page'}`);
...
async function handleMove(invoker, input) {
  ...
  bindCanonicalPageId(input, 'pageId', source, 'move source');
  bindCanonicalPageId(input, 'newParentId', destination, 'move destination');
  if (!sameSpaceKey(source.spaceKey, destination.spaceKey)) {
    throw makeError('CROSS_SPACE_MOVE', 'Cross-space moves are not supported.');
  }
  const rename = hasValue(input.title) ? ` and rename to "${String(input.title).trim()}"` : '';
  return buildResult({
    operation: 'confluence_move',
    input,
    targets: [target('source', source), target('destination', destination)],
    facts: {},
    summary: `Move ${pageSummary(source)} to ${pageSummary(destination)}${rename}?`,
  });
}

async function handleCreateChild(invoker, input) {
  const parent = await resolvePage(invoker, input.parentId, 'parent');
  bindCanonicalPageId(input, 'parentId', parent, 'parent');
  const title = requireText(input.title, 'child title');
  return buildResult({
    operation: 'confluence_create_child',
    input,
    targets: [target('parent', parent)],
    facts: {},
    summary: `Create child "${title}" under ${pageSummary(parent)}${payloadDetails(input, { includeType: true })}?`,
  });
}

function normalizeSpaceRecord(value) {
  if (!value || typeof value !== 'object') {
    throw makeError('MALFORMED_RESULT', 'Preflight space response entries must be objects.');
  }
  const spaceKey = normalizeSpaceKey(value.key ?? value.spaceKey, 'space key');
  const title = requireText(value.name ?? value.title, 'space name');
  return { spaceKey, title };
}

async function handleCreate(invoker, input) {
  const requestedSpace = normalizeSpaceKey(input.spaceKey, 'space key');
  const response = await invokeJson(invoker, 'confluence_space_lookup', { spaceKey: requestedSpace });
  if (response && typeof response === 'object' && response.found === false) {
    throw makeError('TARGET_NOT_FOUND', 'Preflight space target was not found.');
  }
  const destination = normalizeSpaceRecord(response);
  if (!sameSpaceKey(requestedSpace, destination.spaceKey)) {
    throw makeError('TARGET_MISMATCH', 'Preflight space response key does not match the requested target.');
  }
  input.spaceKey = destination.spaceKey;
  return buildResult({
    operation: 'confluence_create',
    input,
    targets: [{ role: 'destination', title: destination.title, spaceKey: destination.spaceKey }],
    facts: {},
    summary: `Create "${hasValue(input.title) ? String(input.title).trim() : 'page'}" in ${destination.title} (SPACE: ${destination.spaceKey})${payloadDetails(input, { includeType: true })}?`,
  });
}
...
function attachmentUploadSummary(input, page) {
  ...
  const uploadComment = hasValue(input.comment) ? `; comment: ${String(input.comment)}` : '';
  const minorEdit = `; minor edit: ${input.minorEdit === true ? 'yes' : 'no'}`;
  return `Upload attachments to ${pageSummary(page)}: ${parts.join(', ')}; total ${totalText}; ${replaceState}${uploadComment}${minorEdit}?`;
}

async function handleSinglePage(invoker, input, operation) {
  const page = await resolvePage(invoker, input.pageId, operation);
  bindCanonicalPageId(input, 'pageId', page, operation);
  let summary;
  if (operation === 'confluence_update') {
    const title = hasValue(input.title) ? `; new title: "${String(input.title).trim()}"` : '';
    summary = `Update ${pageSummary(page)}${title}${payloadDetails(input)}?`;
  } else if (operation === 'confluence_comment_create') {
    const parent = hasValue(input.parent) ? `; parent ${String(input.parent).trim()}` : '; new thread';
    const location = `; location: ${input.location ?? 'footer'}`;
    const hasInlineMetadata = [input.inlineSelection, input.inlineOriginalSelection, input.inlineMarkerRef, input.inlineProperties]
      .some((value) => value !== undefined && value !== null);
    const inlineMetadata = `; inline metadata: ${hasInlineMetadata ? 'yes' : 'no'}`;
    summary = `Create comment on ${pageSummary(page)}${payloadDetails(input)}${parent}${location}${inlineMetadata}?`;
  } else if (operation === 'confluence_property_set') {
    const existing = await findPagedMatch({
      invoker,
    ...
    const bytes = Number.isSafeInteger(input.propertyBytes) && input.propertyBytes >= 0
      ? `; ${input.propertyBytes} bytes`
      : '';
    summary = `Set property ${input.key} on ${pageSummary(page)}${bytes}; replace existing: ${existing ? 'yes' : 'no'}?`;
  } else if (operation === 'confluence_attachment_upload') {
    summary = attachmentUploadSummary(input, page);
  } else {
    summary = `Delete ${pageSummary(page)}?`;
  }
...
async function handleCommentDelete(invoker, input) {
  ...
  bindCanonicalPageId(input, 'pageId', page, 'comment');
  const comment = await invokeJson(invoker, 'confluence_comment_lookup', {
    commentId: input.commentId,
  });
  verifyDirectOwnership(comment, input.commentId, page, 'comment');
...
async function handlePropertyDelete(invoker, input) {
  ...
  bindCanonicalPageId(input, 'pageId', page, 'property');
...
async function handleAttachmentDelete(invoker, input) {
  ...
  bindCanonicalPageId(input, 'pageId', page, 'attachment');
  const attachment = await invokeJson(invoker, 'confluence_attachment_lookup', {
    attachmentId: input.attachmentId,
  });
  verifyDirectOwnership(attachment, input.attachmentId, page, 'attachment');
...
async function handleVersionDelete(invoker, input) {
  ...
  bindCanonicalPageId(input, 'pageId', page, 'versions');
...
function requirePageVersion(page, label) {
  if (!Number.isSafeInteger(page.versionNumber) || page.versionNumber < 1) {
    throw makeError('MALFORMED_RESULT', `Preflight ${label} must include a positive integer version.`);
  }
  return page.versionNumber;
}

function normalizePlannedTreeFingerprint(value) {
  if (typeof value !== 'string' || !/^[a-f0-9]{64}$/.test(value)) {
    throw makeError('MALFORMED_RESULT', 'Preflight copy tree response must include a valid planned tree fingerprint.');
  }
  return value;
}
...
async function handleCopyTreePreview(invoker, input, operation) {
  ...
  bindCanonicalPageId(input, 'sourcePageId', source, 'copy tree source');
  bindCanonicalPageId(input, 'targetParentId', destination, 'copy tree destination');
  ...
  if (!Number.isSafeInteger(childCount) || childCount < 0) {
    throw makeError('MALFORMED_RESULT', 'Preflight copy tree child count must be a non-negative integer.');
  }

  const previewSourceId = requireText(preview.sourcePageId, 'preview source page ID');
  const previewDestinationId = requireText(preview.targetParentId, 'preview destination page ID');
  const sourceVersion = requirePageVersion(source, 'copy tree source');
  const destinationVersion = requirePageVersion(destination, 'copy tree destination');
  if (previewSourceId !== source.pageId || previewDestinationId !== destination.pageId) {
    throw makeError('TARGET_MISMATCH', 'Preflight copy tree preview identities do not match canonical targets.');
  }
  if (Number(preview.sourceVersion) !== sourceVersion || Number(preview.targetParentVersion) !== destinationVersion) {
    throw makeError('TARGET_MISMATCH', 'Preflight copy tree preview versions do not match canonical targets.');
  }
  const plannedTreeFingerprint = normalizePlannedTreeFingerprint(preview.plannedTreeFingerprint);
  const rootTitle = hasValue(preview.rootTitle) ? String(preview.rootTitle).trim() : source.title;
  const totalCreateCount = childCount + 1;
  const plannedRoot = rootTitle === source.title ? '' : ` Planned root title: ${rootTitle}.`;
  const copyOptions = [
    `max depth: ${input.maxDepth ?? 10}`,
    hasValue(input.exclude) ? `exclude: ${input.exclude}` : 'exclude: none',
    `delay: ${input.delayMs ?? 100} ms`,
    `copy suffix: "${input.copySuffix ?? ' (Copy)'}"`,
  ].join('; ');
  const summary = `Copy ${totalCreateCount} pages from ${pageSummary(source)} to ${pageSummary(destination)}${plannedRoot} [${copyOptions}]?`;
  return buildResult({
    operation,
    input,
    targets: [target('source', source), target('destination', destination)],
...
async function handleVersionsPurge(invoker, input, operation) {
  ...
  return buildResult({
    ...
    summary: `Purge ${state.historicalVersions.length} versions from ${pageSummary(page)} [throttle: ${input.throttle ?? 0} seconds]?`,
