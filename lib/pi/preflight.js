...
async function handleSinglePage(invoker, input, operation) {
  ...
  const actionMap = {
    ...
    confluence_attachment_upload: 'Upload attachment on',
  };
  return buildResult({
    operation,
    input,
    ...
    facts: {},
    summary: `${actionMap[operation] || 'Process'} ${pageSummary(page)}?`,
  });
}
...
async function handleCopyTreePreview(invoker, input, operation) {
  ...
  return buildResult({
    ...
    summary: `Copy ${totalCreateCount} pages from ${rootTitle} (ID: ${source.pageId}, SPACE: ${source.spaceKey}) to ${pageSummary(destination)}?`,
