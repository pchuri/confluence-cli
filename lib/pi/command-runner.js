...
function redactText(text, env = process.env) {
  ...
  redacted = redacted.replace(
    /((?:^|[^A-Za-z0-9_-])(?:\\*["'])?authorization(?:\\*["'])?[^\r\n]{0,40}?)(?:basic|bearer)\s+[^\\'"`\s,}\];]+/gi,
    '$1[REDACTED]',
  );
  redacted = redacted.replace(
    /((?:^|[^A-Za-z0-9_-])(?:\\?["'])?cookie(?:\\?["'])?\s*[:=]\s*(?:\\?["'])?)(?!\[REDACTED\])[^'"\r\n,}\]]+/gi,
    '$1[REDACTED]',
  );
...
function redactJsonValue(value, env, depth = 0) {
  if (typeof value === 'string') {
    if (depth < 4) {
      try {
        return JSON.stringify(redactJsonValue(JSON.parse(value), env, depth + 1));
      } catch {
        // Preserve non-JSON strings while redacting credential-shaped text.
      }
    }
    return redactText(value, env);
  }
  if (Array.isArray(value)) {
    return value.map((entry) => redactJsonValue(entry, env, depth));
  }
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [
      key,
      /^(authorization|cookie)$/i.test(key) ? '[REDACTED]' : redactJsonValue(entry, env, depth),
    ]));
  }
  return value;
}
...
function runCommand({
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code, signalCode) => {
      if (mutation && (aborted || timedOut || truncated)) {
        const reason = aborted ? 'was aborted' : timedOut ? 'timed out' : 'produced truncated output';
        settleReject(makeError(ERROR_CODES.UNKNOWN_RESULT, `Confluence CLI mutation result is unknown because execution ${reason}.`, {
          unknownResult: true,
        }));
        return;
      }
      ...
      if (expectJson) {
        try {
          const json = redactJsonValue(JSON.parse(stdout), env);
          settleResolve({
