...
// errSecInteractionNotAllowed: the keychain is locked and no UI is available
// (typically an SSH session).
const INTERACTION_NOT_ALLOWED_STATUS = 36;
...
function getKeychainToken({ host, login } = {}) {
  ...
  if (result.status !== 0) {
    if (result.status === INTERACTION_NOT_ALLOWED_STATUS) {
      warn('macOS Keychain is locked and cannot prompt here; skipping it. Unlock it or set CONFLUENCE_KEYCHAIN=off to silence this.');
...
// Returns { replaced } so callers can tell the user when an existing item
// (possibly shared with another profile on the same host) was overwritten.
function setKeychainToken({ host, login, token } = {}) {
  ...
  const existing = getKeychainToken({ host, login });
  const replaced = existing.token !== undefined && existing.token !== token;
