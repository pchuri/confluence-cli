...
const confluencePlugin = require('./markdown-confluence');
...
class ConfluenceClient {
  constructor(config) {
    ...
    this.markdown = new MarkdownIt().use(confluencePlugin, {
  ...
  async downloadAttachment(pageIdOrUrl, attachmentIdOrAttachment, options = {}) {
    ...
    if (this.authType === 'cookie' && this.cookie) {
      ...
      downloadHeaders.Authorization = this.authType === 'basic' ? this.buildBasicAuthHeader() : `Bearer ${this.token}`;
    }
  ...
  markdownToStorage(markdown) {
    // The markdown-confluence plugin handles direct conversion to storage format
    return this.markdown.render(markdown);
  }

  /**
   * Convert HTML to native Confluence storage format
   * This is now a simplified version that handles basic HTML-to-storage conversion
   * for cases where raw HTML (not markdown) needs to be converted.
   */
   ...
   * @deprecated Use markdownToStorage() instead
   */
  markdownToNativeStorage(markdown) {
    return this.markdownToStorage(markdown);
  }
...
ConfluenceClient.createLocalConverter = function () {
  ...
  instance.markdown = new MarkdownIt().use(confluencePlugin, {
    isCloud: false,
    allowHtmlPassthrough: true
  });
  return instance;
