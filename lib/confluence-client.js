...
class ConfluenceClient {
  ...
  toAbsoluteUrl(pathOrUrl) {
    ...
    // For download paths and other web UI paths, include the webUrlPrefix
    if (pathOrUrl.startsWith('/download/') || pathOrUrl.startsWith('/plugins/')) {
      const normalized = this.webUrlPrefix || '';
      return `${this.protocol}://${this.domain}${normalized}${pathOrUrl}`;
    }
