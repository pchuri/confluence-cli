...
class ConfluenceClient {
   ...
   */
  async listProperties(pageIdOrUrl) {
    const pageId = await this.extractPageId(pageIdOrUrl);
    const response = await this.client.get(`/content/${pageId}/property`);
  ...
  async setProperty(pageIdOrUrl, key, value) {
    ...
    let version = 1;
    try {
      const existing = await this.client.get(`/content/${pageId}/property/${key}`);
    ...
    const response = await this.client.put(`/content/${pageId}/property/${key}`, {
      key,
      value,
