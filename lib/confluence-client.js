...
class ConfluenceClient {
  constructor(config) {
    ...
    this.converter = new MacroConverter({
      ...
      plantumlFormat: config.plantumlFormat,
    });
    ...
    this.client.interceptors.response.use(
      ...
      async error => {
        ...
        // 429 means the server rejected the request before processing, so
        // any method is safe to replay. A 503 can arrive after a proxy or
        // backend already applied a write, so only retry read methods.
        const retryable = status === 429 ||
          (status === 503 && RETRY_SAFE_METHODS.has((requestConfig?.method || '').toLowerCase()));
        if (
          requestConfig &&
          retryable &&
          // A stream body is consumed on first send and cannot be replayed
  ...
  setPlantumlFormat(plantumlFormat) {
    this.converter.plantumlFormat = resolvePlantumlFormat({ plantumlFormat });
  }
      ...
      if (displayMatch) {
        ...
        try {
          ...
          if (!isJsonMode()) {
            ...
            console.error(`Error resolving page ID from display URL${statusSuffix}.`);
          }
        }
  ...
  /**
   * Get metadata for one space.
   */
  async getSpaceMetadata(spaceKey) {
    const response = await returnNullForNotFound(() => (
      this.client.get(`/space/${encodeURIComponent(spaceKey)}`)
    ));
    if (response === null) {
      return null;
    }
    const space = response.data;
    return { key: space.key, name: space.name, type: space.type || null };
  }
  ...
  async getSpaces(maxResults = 500, options = {}) {
    ...
    return this.collectPaginated(
      ({ limit, start }) => this.listSpaces({ limit, start }),
      {
        maxResults: cap,
        pageSize: this.parsePositiveInt(options.pageSize, 500),
        start: this.parsePositiveInt(options.start, 0)
      }
    );
  }
  ...
  async getAllComments(pageIdOrUrl, options = {}) {
    return this.collectPaginated(
      ({ limit, start }) => this.listComments(pageIdOrUrl, {
        limit,
        start,
      ...
      {
        maxResults: this.parsePositiveInt(options.maxResults, null) || null,
        pageSize: this.parsePositiveInt(options.pageSize || options.limit, 25),
        start: this.parsePositiveInt(options.start, 0)
      }
    );
  }
  ...
  /**
   * Get compact ownership metadata for one comment without its body.
   */
  async getCommentMetadata(commentId) {
    const response = await returnNullForNotFound(() => (
      this.client.get(`/content/${encodeURIComponent(commentId)}`, {
        params: { expand: 'container,ancestors' }
      })
    ));
    if (response === null) {
      return null;
    ...
    if (content.type !== 'comment') {
      throw new Error('Expected comment content.');
    }
    return {
      id: String(content.id),
      pageId: content.container?.id == null ? null : String(content.container.id),
      parentId: this.getCommentParentId(content.ancestors) || null,
      title: content.title || `Comment ${content.id}`,
    };
  }
  ...
  async getAllAttachments(pageIdOrUrl, options = {}) {
    return this.collectPaginated(
      ({ limit, start }) => this.listAttachments(pageIdOrUrl, {
        limit,
        start,
      ...
      {
        maxResults: this.parsePositiveInt(options.maxResults, null) || null,
        pageSize: this.parsePositiveInt(options.pageSize || options.limit, 50),
        start: this.parsePositiveInt(options.start, 0)
      }
    );
  }
  ...
  /**
   * Get compact ownership metadata for one attachment without a download URL.
   */
  async getAttachmentMetadata(attachmentId) {
    const response = await returnNullForNotFound(() => (
      this.client.get(`/content/${encodeURIComponent(attachmentId)}`, {
    ...
    return {
      ...
      pageId: content.container?.id == null ? null : String(content.container.id),
      title: content.title,
      mediaType: content.metadata?.mediaType || content.type || '',
      fileSize: content.extensions?.fileSize || 0,
  ...
  async getAllProperties(pageIdOrUrl, options = {}) {
    return this.collectPaginated(
      ({ limit, start }) => this.listProperties(pageIdOrUrl, { limit, start }),
      {
        maxResults: this.parsePositiveInt(options.maxResults, null) || null,
        pageSize: this.parsePositiveInt(options.pageSize || options.limit, 25),
        start: this.parsePositiveInt(options.start, 0)
  ...
  /**
   * Delay before retrying a throttled request: honor Retry-After
   ...
   * `fetchPage({ limit, start })` must resolve to `{ results, nextStart }`.
   */
  async collectPaginated(fetchPage, { maxResults = null, pageSize, start = 0 }) {
    const items = [];
    let hasNext = true;
    while (hasNext) {
      if (maxResults !== null && items.length >= maxResults) break;
      const limit = maxResults === null
        ? pageSize
        : Math.min(pageSize, maxResults - items.length);
      const page = await fetchPage({ limit, start });
    ...
    return maxResults !== null ? items.slice(0, maxResults) : items;
  }
...
ConfluenceClient.createLocalConverter = function (options = {}) {
  const instance = Object.create(ConfluenceClient.prototype);
  instance.converter = new MacroConverter(options);
