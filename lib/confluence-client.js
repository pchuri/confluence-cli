...
class ConfluenceClient {
  ...
  async listAttachments(pageIdOrUrl, options = {}) {
    ...
    const contextPath = response.data?._links?.context;
    const results = Array.isArray(response.data.results)
      ? response.data.results.map((item) => this.normalizeAttachment(item, { contextPath }))
      : [];
  ...
  normalizeAttachment(raw, options = {}) {
    return {
      ...
      downloadLink: this.toAbsoluteUrl(raw._links?.download, options)
    };
  }

  getContextPath() {
    const apiPath = (this.apiPath || '').trim();
    if (!apiPath) {
      return '';
    }

    const restIndex = apiPath.indexOf('/rest/');
    if (restIndex === -1) {
      return '';
    }

    const contextPath = apiPath.slice(0, restIndex);
    return contextPath === '/' ? '' : contextPath;
  }

  toAbsoluteUrl(pathOrUrl, options = {}) {
    if (!pathOrUrl) {
      return null;
    }
    ...
    const contextPath = options.contextPath || this.getContextPath();

    if (contextPath && !normalized.startsWith(`${contextPath}/`) && normalized !== contextPath) {
      return `https://${this.domain}${contextPath}${normalized}`;
    }
